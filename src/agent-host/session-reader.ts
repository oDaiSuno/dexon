...
export function buildSessionContext(entries: SessionEntry[], leafId?: string | null): SessionContext {
  ...
  for (const e of path) {
    if (e.type === "custom" && isChannelSourceType(e.customType)) {
      const marker = parseChannelSourceMarker(e.data);
      if (marker) pendingChannelSource = marker;
      continue;
    }
    if (e.type === "custom" && isChannelSourceCancelledType(e.customType)) {
...
export function entryToUiMessage(entry: SessionEntry): AgentMessage | null {
  switch (entry.type) {
    ...
    case "custom_message":
      if (isChannelAttachmentContextType(entry.customType)) return null;
      return {
        role: "custom",
