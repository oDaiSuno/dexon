...
import { isSessionToolsType, stripToolchainMarkers } from "./session-entry-compat";
...
export function getLegacySessionToolNames(sessionManager: Pick<SessionManager, "getEntries">): string[] | undefined {
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (entry.type !== "custom" || !isSessionToolsType(entry.customType)) continue;
...
export class AgentSessionWrapper {
  ...
  beginExtensionBinding(options: ExtensionBindingOptions = {}): void {
    void this.ensureExtensionsBound(options).catch((err) => {
      console.error(
        "[dexon] failed to dispatch session_start to extensions:",
  ...
  private ensureExtensionsBound(options: ExtensionBindingOptions = {}): Promise<void> {
    ...
    const bindingPromise: Promise<void> = (async () => {
      ...
      this.applyForcedEmptySystemPrompt();
      console.log(`[dexon] session_start dispatched to extensions for session ${this.inner.sessionId}`);
    })()
      .catch((err) => {
        this.extensionBindingError = err;
        console.warn(
          `[dexon] extension binding attempt ${attempt} failed after ${Date.now() - startedAt}ms for session ${this.inner.sessionId}`,
        );
        throw err;
      })
      .finally(() => {
  ...
  private applyToolchainSummary(): void {
    ...
    const base = stripToolchainMarkers(String(this.inner.agent.state.systemPrompt ?? ""));
  ...
  async runExternalTurn(params: {
    ...
    return this.enqueueTurn(async () => {
      ...
      try {
        this.inner.sessionManager.appendCustomEntry("dexon-channel-source", {
          runId: params.runId,
        ...
        try {
          this.inner.sessionManager.appendCustomEntry("dexon-channel-source-cancelled", { runId: params.runId });
        } catch {
      ...
      const teardown = async () => {
        if (abort) {
          try {
            ...
            console.warn(
              `[dexon] session abort failed during ${reason} for ${this.sessionId}: ${error instanceof Error ? error.name : "UnknownError"}`,
            );
          }
        }
        ...
        try {
          ...
          console.warn(
            `[dexon] session waitForIdle failed during ${reason} for ${this.sessionId}: ${error instanceof Error ? error.name : "UnknownError"}`,
          );
        }
        try {
          await agent.dispose?.();
          ...
          console.warn(
            `[dexon] session dispose failed during ${reason} for ${this.sessionId}: ${error instanceof Error ? error.name : "UnknownError"}`,
          );
        }
      };
      ...
      try {
        await Promise.race([
          ...
          new Promise<void>((resolve) => {
            timeout = setTimeout(() => {
              console.warn(`[dexon] session teardown timed out after ${timeoutMs}ms during ${reason}`);
              resolve();
            }, timeoutMs);
          }),
        ]);
